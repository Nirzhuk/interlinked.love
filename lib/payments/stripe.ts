...
export async function createCustomerPortalSession(couple: Couple) {
	if (!couple.stripeCustomerId || !couple.stripeProductId) {
		redirect("/pricing");
	}
	...
	if (configurations.data.length > 0) {
		...
		const product = await stripe.products.retrieve(couple.stripeProductId);
		if (!product.active) {
			throw new Error("Couple's product is not active in Stripe");
		}
		...
		if (prices.data.length === 0) {
			throw new Error("No active prices found for the couple's product");
		}
...
export async function handleSubscriptionChange(subscription: Stripe.Subscription) {
	...
	const couple = await getCoupleByStripeCustomerId(customerId);

	if (!couple) {
		console.error("Couple not found for Stripe customer:", customerId);
		return;
	}

	if (status === "active" || status === "trialing") {
		const plan = subscription.items.data[0]?.plan;
		await updateCoupleSubscription(couple.id, {
